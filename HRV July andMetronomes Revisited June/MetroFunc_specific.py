...
def DRWLOG(data, lbls):
    ...
    ax0.plot(x_ann, y_ann, 'r-')
    ax0.annotate('y = '+ str(np.round(fit[0],2)) +' x ' + str(np.round(fit[1],2)), xy=(0.1, 0.35), xycoords=ax0.transAxes)
    ax0.annotate('R-squared : '+ str(np.round(fit[2]**2,2)), xy=(0.1, 0.25), xycoords=ax0.transAxes)
    ax0.grid(True)
    ax0.set_xticks(np.arange(-5, 1, 0.5))
    ax0.set_xticklabels(np.arange(-5, 1, 0.5))
    ax0.set_xlim((-5,0))
    plt.show()
