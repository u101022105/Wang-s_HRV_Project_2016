...
def draw(data, title, mode, lbl):
    ...
    if mode == 0:
        ...
        [sizex, sizey] = lbl[6]
        fig0.set_size_inches(sizex, sizey)
        if lbl[3] == 1:
            '''
            lbl[4] = xsteps for xticks
            lbl[5] = ysteps for yticks
            '''
            xstep = lbl[4]
            ystep = lbl[5]
            xt = np.arange(x[0], x[-1], step= xstep)
            yt = np.arange(round(y.min(), 5), round(y.max(), 5), step= ystep)
            ax0.set_xticks(xt)
