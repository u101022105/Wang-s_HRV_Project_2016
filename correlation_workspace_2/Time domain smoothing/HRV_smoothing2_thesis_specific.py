...
def draw(data, title, mode, lbl):
    ...
    if mode == 0:
        ...
        if type(lbl) == list:
            ...
            ax0.set_xlabel(xlbl)
            ax0.set_ylabel(ylbl)
            #ax0.set_xticks(np.round(np.arange(0,x[-1], 5000), 0))
            #ax0.set_xticklabels(np.round(np.arange(0,x[-1], 5000), 0))
            ax0.grid(True)
            ax0.plot(x,y,'.r')
            ax0.plot(x,y,'-b')
        else:
        ...
        if lbl[3] == 1:
            xt = np.arange(x[0], x[-1]+ 1000, step= 1000)
            xtl = [int(i) for i in xt]
            yt = np.arange(np.around(y.min(), 2)-0.02, np.around(y.max(), 2)+0.02, step= 0.01)
            ytl = [format(j, '.3f') for j in yt]
            ax0.set_xlim((x[0], x[-1]))
            #ax0.set_ylim((0.54,0.91))
            ax0.set_xticks(xt)
            ax0.set_xticklabels(xtl,rotation ="vertical")
            ax0.set_yticks(yt)
            ax0.set_yticklabels(ytl)
        ...
        fig0.set_size_inches(20, 8)
        if lbl[3] == 1:
            xt = np.arange(x[0], x[-1]+ 1000, step= 1000)
            xtl = [int(i) for i in xt]
            #yt = np.arange(np.around(y.min(), 5), np.around(y.max(), 5), step= 0.0001)
            yt = np.arange(np.around(y.min(), 4)-0.0001, np.around(y.max(), 4)+0.0001, step= 0.0001)
            ytl = [format(j, '.4f') for j in yt]
            ax0.set_xlim((x[0], x[-1]))
            ax0.set_ylim((yt[0],yt[-1]))
            ax0.set_xticks(xt)
            ax0.set_xticklabels(xtl,rotation ="vertical")
            ax0.set_yticks(yt)
            ax0.set_yticklabels(ytl)
